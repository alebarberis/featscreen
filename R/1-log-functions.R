...
#'logging system in \code{featscreen}.
#'
...
newLogger <- function(
  ...
  stopifnot("'path' must be a character string. Please, check your input.\n" = is.character(path))
  stopifnot("'con' must be 'NULL' or of class 'connection'. Please, check your input.\n" = (is.null(con) || is.connection(con)))
  stopifnot("'verbose' must be a boolean. Please, check your input.\n" = is.logical(verbose))
  stopifnot("'level' must be a valid character string. Please, check your input.\n" = (is.character(level) && (level %in% c("OFF", "INFO", "DEBUG", "TRACE", "ALL")) ))
...
Logger <- function(
  ...
  #----------------------------------------------------------------------#
  return(
    newLogger(path = path, con = con, verbose = verbose, level = level)
  )
}
...
#'Check class
#'
#'@description Function to check if input object is of class \code{\link{Logger}}.
#'
#' @param x object to be tested
#'
#' @return Logical, whether \code{x} is of class \code{\link{Logger}}.
#'
#' @author Alessandro Barberis
#'
#' @keywords internal
is.Logger <- function(x){
  #check if has class Logger
  out = inherits(x, "Logger")
  if(isTRUE(out)){
    out = tryCatch(
      expr = {#get slots
        con = getCon(x)
        verbose = getVerbose(x)
        path = getPath(x)
        level = getLevel(x)

        #check class
        if(isFALSE(is.character(path))){return(FALSE)
        }else if(isFALSE(is.null(con) || is.connection(con))){return(FALSE)
        }else if(isFALSE(is.logical(verbose))){return(FALSE)
        }else if(isFALSE(is.character(level) && (level %in% c("OFF", "INFO", "DEBUG", "TRACE", "ALL")) )){return(FALSE)
        }else{return(TRUE)}
      },
      error = function(e){
        return(FALSE)
